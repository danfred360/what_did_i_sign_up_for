...
async def search_by_collection(collection_id: int, query: str, num_results: int = 5):
    search = SearchProvider()
    try:
...
async def search_by_file_class(file_class_id: int, query: str, num_results: int = 5):
    search = SearchProvider()
    try:
